...
public class ResultsServiceImpl extends ServiceImpl<ResultsDao, Results> implements ResultsService {
    ...
    public Result get_result(GetResultsDTO getResultsDTO, HttpSession session) {
        ...
        for(String groupid:getResultsDTO.getGroups()) {
            ...
            //为空，查询数据库
            result=query().eq("groupT",groupid).one();
            //检查是否为空
            if(result==null) {
                //为空，表明无论缓存还是数据库中都没有，除非传入参数错了才会出现这个情况，continue
                continue;
            }
            //不为空，转换为map
    ...
    public Result set_result(SetReturnResultsDTO setResultsDTO, HttpSession session) {
        ...
        for(Results result:setResultsDTO.getSet_return_results()) {
            //对每一个更改，检查单号和结果不为空
            if(result.getGroupT()==null||result.getResult()==null){
                result.setResult("false");
                continue;
            }
            //从数据库按照组号查找一个对象
            Results oldresult = query().eq("groupT",result.getGroupT()).one();
            //判断是否为空
            if(oldresult==null) {
            ...
            //判断更改有无地点
            if (result.getDetectunit()!=null) {
                //有设置地点为更改的地点
                oldresult.setDetectunit(result.getDetectunit());
            }else {
                //无，设置为传过来的统一地点
                if(setResultsDTO.getMedical_name()!=null){
